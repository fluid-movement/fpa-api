...
export function startScheduler(store: Store): () => void {
	...
	const every = (intervalMs: number, task: () => Promise<unknown>, label: string) => {
	...
	every(
		config.refresh.directoryProbeIntervalMs,
		() => store.probeDirectory(),
		'directory probe'
	);
	every(
		config.refresh.manifestProbeIntervalMs,
		() => store.probeManifest(),
		'manifest probe'
	);
	...
	console.log(
		`[scheduler] directory probe ${config.refresh.directoryProbeIntervalMs}ms, ` +
