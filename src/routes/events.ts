...
const listRoute = createRoute({
	...
	summary: 'List events',
	description:
		'Every event known to the upstream event-summary service, newest first. ' +
...
events.openapi(listRoute, (c) => {
	...
	if (from) items = items.filter((e) => e.startDate !== null && e.startDate >= from);
	if (to) items = items.filter((e) => e.startDate !== null && e.startDate <= to);
	if (division) items = items.filter((e) => e.divisions.includes(division));
	if (hasResults === 'true') items = items.filter((e) => e.resultCount > 0);
	if (hasResults === 'false') items = items.filter((e) => e.resultCount === 0);
...
events.openapi(resultsRoute, (c) => {
	...
	if (!index.events.has(eventId)) throw notFound('Event', eventId);

	let results = index.resultsByEvent.get(eventId) ?? [];
	if (division) results = results.filter((r) => r.division === division);
