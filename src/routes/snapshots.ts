...
const listRoute = createRoute({
	...
	description:
		'Rankings and ratings are published upstream as dated snapshots. This lists them newest ' +
...
snapshots.openapi(listRoute, (c) => {
	...
	if (division) items = items.filter((s) => s.division === division);
...
const detailRoute = createRoute({
	...
	description:
		'Fetched from upstream on demand rather than held in memory: there are around 100 snapshots ' +
...
snapshots.openapi(detailRoute, async (c) => {
	...
	if (!snapshot) throw notFound('Snapshot', key);
	...
	const { rankings, ratings } = buildPointsSeries(
		snapshotToPoints(key, payload),
		index.results
	);

	return c.json(
		{ snapshot, rankings: [...rankings.values()], ratings: [...ratings.values()] },
		200
	);
});
