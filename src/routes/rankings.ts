...
const seriesRoute = createRoute({
	...
	description:
		'Call this to discover valid `series` values rather than hardcoding them. ' +
...
const currentRoute = createRoute({
	...
	description:
		'Cumulative tournament points. Defaults to `ranking-open`. Each entry carries a `breakdown` ' +
...
rankings.openapi(currentRoute, (c) => {
	...
	if (!series) {
		throw httpError(
			404,
			`Unknown ranking series "${key}". Available: ${[...index.rankings.keys()].join(', ')}`
		);
	}
