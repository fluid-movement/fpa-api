...
const listRoute = createRoute({
    ...
    description: 'Only canonical players are listed. Upstream stores duplicate entries for the same person ' +
...
function computeStats(index, playerId) {
    ...
    for (const placement of placements) {
        ...
        if (placement.place === 1)
            wins++;
        if (placement.place !== null && placement.place <= 3)
            podiums++;
        if (placement.eventDate)
            dates.push(placement.eventDate);
    }
    dates.sort();
    return {
...
function currentRankings(index, playerId) {
    const rankings = [];
...
function currentRatings(index, playerId) {
    const ratings = [];
...
const profileRoute = createRoute({
    ...
    description: 'Returns the player, aggregate career stats, their current standing in every ranking and ' +
