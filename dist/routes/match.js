...
const MATCH_NOTE = 'Results are suggestions ranked by similarity, never an automatic link. fpa-events and the ' +
...
match.openapi(eventMatchRoute, (c) => {
    ...
    if (hasResults !== 'false')
        candidates = candidates.filter((e) => e.resultCount > 0);
    const ranked = rankCandidates(candidates, { name, startDate: startDate ?? null }, { name: (e) => e.name, startDate: (e) => e.startDate });
    return c.json({
...
match.openapi(playerMatchRoute, (c) => {
    ...
    const ranked = rankCandidates([...index.players.values()], { name }, { name: (p) => p.fullName, startDate: () => null });
    return c.json({
        query: { name },
        candidates: ranked.map((candidate) => ({
