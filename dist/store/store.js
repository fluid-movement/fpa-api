import { buildIndex, summarizePoolLocks } from '../domain/index-builder.js';
...
export class Store {
    ...
    #lastDirectorySignature = null;
    #lastManifestSignature = null;
    /**
    ...
    async initialize() {
        ...
        if (snapshot) {
            ...
            this.#origin = 'snapshot';
            console.log(`[store] restored snapshot from ${new Date(snapshot.fetchedAt).toISOString()} ` +
    ...
    async #doRefresh() {
        ...
        const [players, events, results, manifest, points, directory] = await Promise.all([
            sources.players().then((v) => (this.#record('players', null), v), (e) => (this.#record('players', e), null)),
            sources.events().then((v) => (this.#record('events', null), v), (e) => (this.#record('events', e), null)),
            sources.results().then((v) => (this.#record('results', null), v), (e) => (this.#record('results', e), null)),
            sources.manifest().then((v) => (this.#record('manifest', null), v), (e) => (this.#record('manifest', e), null)),
            sources.latestPoints().then((v) => (this.#record('points', null), v), (e) => (this.#record('points', e), null)),
            sources.directory().then((v) => (this.#record('directory', null), v), (e) => (this.#record('directory', e), null))
        ]);
        ...
        const liveLocks = await this.#fetchLiveLocks(directory ?? previous?.directory ?? null, previous?.liveLocks);
        ...
        try {
            ...
            this.#corpus = corpus;
            this.#index = index;
            this.#lastDirectorySignature = this.#signDirectory(corpus.directory, this.#cachedEventDataVersions(corpus.directory));
     ...
     */
    async #fetchLiveLocks(directory, previous) {
        ...
        await Promise.all(keys.map(async (key) => {
            ...
            try {
                ...
                if (version !== null)
                    this.#eventDataVersions.set(key, version);
            }
            catch (error) {
                failed = error;
    ...
    async probeDirectory() {
        try {
            ...
            const signature = this.#signDirectory(directory, await this.#probeEventDataVersions(directory));
            if (signature === this.#lastDirectorySignature)
                return false;
     ...
     */
    #signDirectory(directory, versions) {
    ...
    #cachedEventDataVersions(directory) {
        const versions = {};
     ...
     */
    async #probeEventDataVersions(directory) {
        const keys = (directory.eventDirectory ?? [])
            .map((entry) => entry.eventKey)
            .filter((key) => typeof key === 'string' && key.length > 0)
            .sort();
